...
try: 
    for message in list(messages):
        if subject in message.Subject:
            ...
            msgdate = msgdate[:10]
            new_dir_name = os.makedirs(attachment_file_path + '\\' + time.strftime(msgdate), exist_ok=True) #Creates directory based on Date Received
            ...
            try: 
                ...
                for attachment in message.Attachments: 
                    new_fid_name = os.makedirs(new_dir_name + '\\' + keyword, exist_ok=True) #Creates a directory based on the Keyboard
                    new_fid_name = pathlib.Path(new_dir_name + '\\' + keyword, exist_ok=True) 
                    new_fid_name = str(new_fid_name)
                    attachment.SaveASFile(os.path.join(new_fid_name + '\\' +"FID " + keyword + " " +  (attachment.FileName))) #saves attachment 
